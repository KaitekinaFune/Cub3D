...
/*   Updated: 2021/01/23 12:45:45 by lflint           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	check_texture(cub, line, mode);
	if (mode == 1)
		cub->file.textures[0] = line;
	else if (mode == 2)
		cub->file.textures[1] = line;
	else if (mode == 3)
		cub->file.textures[2] = line;
	else if (mode == 4)
		cub->file.textures[3] = line;
	else if (mode == 5)
		cub->file.textures[4] = line;
	else if (mode == 6)
		cub->file.f_clr = line;
	else if (mode == 7)
		cub->file.c_clr = line;
	...
	get_file(t_cub *cub, char *map_name)
{
	...
	if (!(check_ext(map_name, ".cub")))
		cub_error(1, cub, "Invalid File Extension\n");
	if ((fd = open(map_name, O_RDONLY)) < 0)
		cub_error(15, cub, "Bad Map File\n");
